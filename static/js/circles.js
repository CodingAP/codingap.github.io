...
window.addEventListener('resize', () => {
    ...
    for (let i = 0; i < circleDivs.length; i++) {
        circleDivs[i].div.style.left =
            `${circleDivs[i].position.x * width + (circleDivs[i].left ? 0 : mainContainer.clientWidth + width)}px`;
        circleDivs[i].div.style.top =
            `${circleDivs[i].position.y * (document.documentElement.scrollHeight - 100)}px`;
